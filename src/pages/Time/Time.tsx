...
export const Time: React.FC = () => {
  ...
  const resultHour =
    time.resultTime !== null
      ? time.resultTime.t_hour
      : '';
  const resultMin =
    time.resultTime !== null
      ? time.resultTime.t_min
      : '';
  const resultSec =
    time.resultTime !== null ? time.resultTime.t : '';

  const resultString = `${resultHour ? resultHour : 0}ч ${resultMin ? resultMin : 0}мин ${resultSec ? resultSec : 0}сек`;
        ...
        <div className="time__container">
          {showResult && (
            <div className="time__result">
              <p>Время покрытия: <span>{resultString}</span></p>
            </div>
          )}
          <Form
            name="time"
